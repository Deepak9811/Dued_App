...
import {API, Layout} from '../api/API';
...
/*
    ...
    render = ()=>{
        ...
        let {index, routes,revealed} = this.state;
...
*/
        ...
        return (<View style={{marginTop:0,marginLeft:10,marginRight:10}} >
            ...
            <Heading key={"fieldtxt"+field.id} size={this.props.parentIsSection?-4:-3}
                     ...
                     rightText={this.props.plan._result?(isNaN(this.props.plan._result)?0:parseFloat(this.props.plan._result).toFixed(2))+'%':'0%'}
                     revealed={this.state.revealed}
                     onReveal={()=>{
                         this.setState({revealed:!this.state.revealed},()=>{
                             APIEvents.call('resetformsizes');
                         })
                     }}
                     style={{marginLeft:-10,paddingTop:0,paddingBottom:18}}>{(field.name)}</Heading>


            {this.state.revealed?<FormRenderer
